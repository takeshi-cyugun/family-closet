...
import { loginOwner } from "../actions/loginOwner";
...
type OwnerLoginFormProps = {
  t: HomeDictionary["ownerLoginForm"];
};

// 代表者ログイン: メールアドレス＋パスワードのみ。ファミリーIDはサーバー側でCookieに解決するため入力不要。
export function OwnerLoginForm({ t }: OwnerLoginFormProps) {
  ...
  return (
    <form className="flex flex-col gap-3" onSubmit={handleSubmit} noValidate>
      <div className="flex flex-col gap-1">
        <label htmlFor="ownerLoginEmail" className="text-sm font-medium text-ink">
          {t.email}
        </label>
        <input
          id="ownerLoginEmail"
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="ownerLoginPassword" className="text-sm font-medium text-ink">
          {t.password}
        </label>
        <input
          id="ownerLoginPassword"
          type="password"
          value={password}
